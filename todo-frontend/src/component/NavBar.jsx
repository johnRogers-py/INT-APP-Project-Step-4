...
function NavBar({ cartCount, searchQuery, onSearchChange, onCartToggle, onReturnsOrders, categories, activeCategory, onCategoryChange }) {
  return (
    <header className={styles.header}>
      ...
      <nav className={styles.primaryNav} aria-label="Primary navigation">
        <div className={styles.primaryInner}>
          ...
          {/* Account & Lists */}
          <button className={styles.navBtn} aria-label="Account and lists">
      ...
      <nav className={styles.secondaryNav} aria-label="Secondary navigation">
        <div className={styles.secondaryInner}>
          ...
          {["Today's Deals", "Customer Service", "Registry", "Gift Cards", "Sell", "Prime"].map(item => (
            <button key={item} className={styles.secNavLink}>{item}</button>
          ))}
          <button className={styles.secNavLinkHighlight}>🛡 Protection Plans</button>
        </div>
      </nav>
    </header>
  );
